...
function addReplied(uri: string) {
  repliedSet.add(uri);
...
function hasVideoEmbed(record: any): boolean {
  ...
  for (const e of embeds) {
    ...
    if (e['$type'] === 'app.bsky.embed.recordWithMedia' && e.record?.embed) {
      ...
      if (nested['$type'] === 'app.bsky.embed.video#view' || nested['$type'] === 'app.bsky.embed.video') return true;
    }
  }
...
function buildDownloadReply(postUrl: string) {
...
function extractPostUrlFromUri(uri: string, authorHandle: string): string {
