...
class BlueskyDownloader:
    ...
    def __init__(self, session_headers: Optional[Dict[str, str]] = None) -> None:
        ...
        self.ydl_opts: Dict[str, Any] = {
            ...
            "format": (
                ...
                "best[ext=mp4]/bv*+ba/bestvideo+bestaudio/best"
            ),
    ...
    def extract(self, url: str) -> ExtractionResult:
        ...
        video_url = self._select_direct_video(info)
        if not video_url:
    ...
    def _select_direct_video(self, info: Dict[str, Any]) -> Optional[str]:
        ...
        def is_progressive(fmt: Dict[str, Any]) -> bool:
            return (
                ...
                and "m3u8" not in (fmt.get("protocol") or "")
            )

        progressive = [fmt for fmt in formats if is_progressive(fmt)]
        if progressive:
            best = max(progressive, key=lambda f: f.get("height") or 0)
            return best.get("url")

        mp4_only = [fmt for fmt in formats if fmt.get("ext") == "mp4"]
        if mp4_only:
            best = max(mp4_only, key=lambda f: f.get("height") or 0)
            return best.get("url")
